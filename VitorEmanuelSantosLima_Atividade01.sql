...
SELECT  cantor.nome_cantor, count(gravacao.cod_cantor) as num_gravacoes 
FROM cantor JOIN gravacao ON cantor.cod_cantor = gravacao.cod_cantor 
GROUP BY nome_cantor HAVING num_gravacoes = 1 


#Questao 2

SELECT DISTINCT cantor.nome_cantor, count(gravacao.cod_gravadora) as num_gravadoras
FROM cantor JOIN gravacao ON cantor.cod_cantor = gravacao.cod_cantor 
GROUP BY nome_cantor

#Questao 3

SELECT cantor.nome_cantor, ROUND(AVG(musica.duracao), 4) as media
FROM cantor
JOIN  gravacao ON  cantor.cod_cantor = gravacao.cod_cantor
JOIN musica ON gravacao.cod_musica = musica.cod_musica
GROUP BY nome_cantor 


#Questao 4

SELECT cantor.nome_cantor, gravadora.cod_gravadora
FROM cantor
JOIN  gravacao ON  cantor.cod_cantor = gravacao.cod_cantor
JOIN  gravadora ON  gravacao.cod_gravadora = gravadora.cod_gravadora
GROUP BY cantor.nome_cantor HAVING gravadora.cod_gravadora != 1
...
JOIN  gravacao ON  cantor.cod_cantor = gravacao.cod_cantor
JOIN  musica ON  gravacao.cod_musica = musica.cod_musica
WHERE gravacao.data_gravacao LIKE "2004%"
...
SELECT pessoa.nome_pessoa, fone.numero
FROM pessoa
JOIN  fone ON  pessoa.cod_pessoa = fone.cod_fone
